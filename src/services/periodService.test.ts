...
import {
  ...
  deletePeriod,
  clearAllPeriods,
} from './periodService.ts';
import { resetDB } from './db.ts';
import { getSettings, saveSettings, resetSettings } from './settingsService.ts';
...
describe('createPeriod', () => {
  ...
  it('throws VALIDATION_ERROR when startDate is missing', async () => {
    // @ts-expect-error — deliberately omitting required startDate to test validation
  ...
  it('throws VALIDATION_ERROR for invalid flow value', async () => {
    await expect(
      // @ts-expect-error — deliberately passing invalid flow value to test validation
  ...
  it('throws VALIDATION_ERROR for mood out of range', async () => {
    await expect(
      // @ts-expect-error — deliberately passing out-of-range mood to test validation
