...
export default function ImportExportPage() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > MAX_IMPORT_FILE_SIZE) {
      showToast({ type: 'error', message: 'File is too large (max 10 MB). Export a smaller backup.' });
      return;
    }
    ...
    reader.onload = (event: ProgressEvent<FileReader>) => {
      ...
      try {
        ...
        showToast({ type: 'error', message: 'Could not read file — make sure it is a valid JSON file.' });
        return;
      }

      const { valid, errors } = validateImportShape(parsed);
      if (!valid) {
        showToast({ type: 'error', message: `Invalid file: ${errors[0]}` });
        return;
      }

      setPendingPayload(parsed as ExportPayload);
      setImportModalOpen(true);
    };
    reader.onerror = () => {
      showToast({ type: 'error', message: 'Failed to read the file. Please try again.' });
    };
  ...
  // -------------------------------------------------------------------------
  async function handleImportStrategy(strategy: 'overwrite' | 'merge') {
    ...
    try {
      ...
      showToast({
        type: 'success',
        message: `Import complete. ${count} period${count !== 1 ? 's' : ''} ${strategy === 'overwrite' ? 'restored' : 'merged'}.`,
      });
    } catch {
      showToast({ type: 'error', message: 'Import failed. Your existing data was not changed.' });
    } finally {
  ...
  async function handleClearConfirm() {
    ...
    try {
      ...
      showToast({ type: 'error', message: 'Failed to clear data. Please try again.' });
    } finally {
      setClearing(false);
    }
  }
